                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                onclick="toogleStatusModal(event,'mail-status','place-order-on.png','place-order-off.png','{{translate('Want_to_enable_the')}} <strong>{{translate('Campaign_Join_Approval ')}}</strong> {{translate('mail')}} ?','{{translate('Want_to_disable_the')}} <strong>{{translate('Campaign_Join_Approval ')}}</strong> {{translate('mail')}} ?',`<p>{{translate('If_enabled,_restaurants_will_receive_an_email_when_Admin_approves_their_Campaign_Join_Request.')}}</p>`,`<p>{{translate('If_disabled,_restaurants_will_not_receive_any_confirmation_email_on_Campaign_Join_Request.')}}</p>`)" id="mail-status" {{$mail_status == '1'?'checked':''}}>

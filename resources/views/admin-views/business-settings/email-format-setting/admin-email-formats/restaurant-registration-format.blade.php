                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input" onclick="toogleStatusModal(event,'mail-status','place-order-on.png','place-order-off.png','{{translate('Want_to_enable_the')}} <strong>{{translate('New_Restaurant_Registration_mail')}}</strong> ?','{{translate('Want_to_disable_the')}} <strong>{{translate('New_Restaurant_Registration_mail')}}</strong> ?',`<p>{{translate('If_enabled,_the_admin_will_get_an_automated_email_when_a_restaurant_registers.')}}</p>`,`<p>{{translate('If_disabled,_the_admin_will_not_get_an_automated_email_when_a_restaurant_registers')}}</p>`)" id="mail-status" {{$mail_status == '1'?'checked':''}}>

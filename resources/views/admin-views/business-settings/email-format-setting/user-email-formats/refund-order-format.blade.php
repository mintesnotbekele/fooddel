                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input" onclick="toogleStatusModal(event,'mail-status','place-order-on.png','place-order-off.png','{{translate('Want_to_enable_the')}} <strong>{{translate('Refund_Order')}}</strong> {{translate('mail')}} ?','{{translate('Want_to_disable_the')}} <strong>{{translate('Refund_Order')}}</strong> {{translate('mail')}} ?',`<p>{{translate('If_enabled,_Customers_will_get_an_automated_email_when_they_receive_a_refund.')}}</p>`,`<p>{{translate('If_disabled,_Customers_will_not_receive_any_mail_on_Refund_Orders.')}}</p>`)" id="mail-status" {{$mail_status == '1'?'checked':''}}>

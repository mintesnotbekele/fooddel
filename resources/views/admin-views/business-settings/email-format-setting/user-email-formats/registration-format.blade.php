                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input" onclick="toogleStatusModal(event,'mail-status','place-order-on.png','place-order-off.png','{{translate('Want_to_enable_the')}} <strong>{{translate('New_Customer_Registration')}}</strong> {{translate('mail')}} ?','{{translate('Want_to_disable_the')}} <strong>{{translate('New_Customer_Registration')}}</strong> {{translate('mail')}} ?',`<p>{{translate('If_enabled,_customers_will_receive_a_confirmation_email_that_their_registration_was_successful.')}}</p>`,`<p>{{translate('If_disabled,_customers_won’t_receive_any_registration_confirmation_email.')}}</p>`)" id="mail-status" {{$mail_status == '1'?'checked':''}}>

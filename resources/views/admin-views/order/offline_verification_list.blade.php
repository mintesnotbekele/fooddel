        ...
        <div class="modal fade" id="verifyViewModal-{{ $key }}" tabindex="-1" aria-labelledby="verifyViewModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="d-flex align-items-center flex-column gap-3 text-center">
                    ...
                    <p class="text-danger mb-2 mt-2">{{ translate('Please_Check_&_Verufy_the_payment_information_weather_it_is_correct_or_not_before_confirm_the_order.') }}</p>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="btn--container justify-content-end mt-3">
                            ...
                            @elseif ($order?->offline_payments->status == 'denied')
                            <button type="button" onclick="cancelled_status({{ $order['id'] }})" class="btn btn-danger btn-outline-danger mb-2">{{translate('Cancel_Order')}}</button>

                            <button type="button" onclick="route_alert('{{ route('admin.order.offline_payment', [ 'id' => $order['id'], 'verify' => 'switched_to_cod', ]) }}','{{ translate('messages.Make_the_payment_verified_for_this_order') }}')"  class="btn btn-primary btn-outline-primary mb-2">{{translate('Continue_with_COD')}}</button>

                            @endif

                            <button type="button" onclick="route_alert('{{ route('admin.order.offline_payment', [ 'id' => $order['id'], 'verify' => 'yes', ]) }}','{{ translate('messages.Make_the_payment_verified_for_this_order') }}')" class="btn btn-primary mb-2">{{translate('Yes,_Payment_Received')}}</button>
                        </div>
                        @endif
                    </div>
                </div>

                        ...
                        <div class="card-body pt-2">
                            ...
                            <div class="row g-3">
                                ...
                                @if ($data)
                                    ...
                                    <div class="col-3">
                                    <img class="img__aspect-1 rounded border w-100" data-toggle="modal" data-target="#imagemodal{{ $key }}" onerror="this.src='{{ asset('assets/admin/img/160x160/img2.jpg') }}"
                                    src="{{asset('storage/app/refund').'/'.$img}}">
                                </div>
                                    ...
                                    @else
                                    <div class="col-3">
                                        <img class="img__aspect-1 rounded border w-100" onerror="this.src='{{ asset('assets/admin/img/160x160/img2.jpg') }}"
                                    ...
                                    @foreach ($data as $key => $img)
                                        <div class="col-3">
                                            <img class="img__aspect-1 rounded border w-100" data-toggle="modal"
                                                ...
                                                src="{{ asset('storage/app/order') . '/' . $img }}">
                                        </div>
                                        <div class="modal fade" id="imagemodal{{ $key }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <img src="{{ asset('storage/app/' . 'order/' . $img) }}"
                                                            class="initial--22 w-100">
                                                    </div>
                                                    <div class="modal-footer">
                                                        <a class="btn btn-primary"
                                                            href="{{ route('admin.file-manager.download', base64_encode('order/' . $img)) }}"><i
                                    ...
                                    <div class="d-flex align-items-center gap-2 justify-content-between">
                                            <div class="dm_list_selected media gap-2" data-id="{{ $selected_delivery_man['id'] }}">
                                                <img class="avatar avatar-60 rounded-10"
                                                    ...
                                                    src="{{ asset('storage/app/delivery-man') }}/{{ $selected_delivery_man['image'] }}"
                                        ...
                                        <li class="list-group-item">
                                            <div class="d-flex align-items-center gap-2 justify-content-between">
                                                    ...
                                                    @if ($order->delivery_man_id )
                                                        ...
                                                        onclick="addDeliveryMan('{{ $dm['id'] }}')">{{ translate('messages.Reassign') }}</a>
                                                    @else
                                                        <a class="btn btn-primary btn-xs float-right"
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="btn--container justify-content-end mt-3">
                            ...
                            @elseif ($order?->offline_payments->status == 'denied')
                            <button type="button" onclick="cancelled_status()" class="btn btn-danger btn-outline-danger mb-2">{{translate('Cancel_Order')}}</button>
                                <button type="button" onclick="route_alert('{{ route('admin.order.offline_payment', [ 'id' => $order['id'], 'verify' => 'switched_to_cod', ]) }}','{{ translate('messages.Make_the_payment_verified_for_this_order') }}')" onclick=""  class="btn btn-primary btn-outline-primary mb-2">{{translate('Continue_with_COD')}}</button>
                            @endif
        ...
        function initializeGMap() {
            ...
            @if ($order->restaurant)
                var Restaurantmarker = new google.maps.Marker({
                    ...
                    icon: "{{ asset('assets/admin/img/restaurant_map_1.png') }}"
                });

                google.maps.event.addListener(Restaurantmarker, 'click', (function(Restaurantmarker) {
                    return function() {
                        infowindow.setContent(
                            `<div class='float--left'><img class='js--design-1' onerror="this.src='{{ asset('assets/admin/img/160x160/img1.jpg') }}'"  src='{{ asset('storage/app/restaurant/' . $order->restaurant->logo) }}'></div><div class='text-break float--right p--10px'><b>{{ Str::limit($order->restaurant->name, 15, '...') }}</b><br/> {{ $order->restaurant->address }}</div>`
                        );
            ...
            for (var i = 0; i < deliveryMan.length; i++) {
                if (deliveryMan[i].lat) {
                    ...
                    map.fitBounds(dmbounds);
                    var d_man = "{{ $order?->delivery_man_id ?? 0 }}" ;
                    if ( deliveryMan[i].id ==  d_man){
                        var dm_default_image=  "{{ asset('assets/admin/img/delivery_boy_map_2.png') }}";
                    } else {
                        var dm_default_image=  "{{ asset('assets/admin/img/delivery_boy_map_1.png') }}";
                    }
                    ...
                    google.maps.event.addListener(marker, 'click', (function(marker, i) {
                        return function() {
                            infowindow.setContent(
                                `<div class='float--left'><img class='js--design-1 mt-2' onerror="this.src='{{ asset('assets/admin/img/160x160/img1.jpg') }}'"   src='{{ asset('storage/app/delivery-man') }}/` +
                                deliveryMan[i].image +
            ...
            function initializegLocationMap() {
                ...
                @if ($order->delivery_man && $order->dm_last_location)
                    var dmmarker = new google.maps.Marker({
                        ...
                        icon: "{{ asset('assets/admin/img/delivery_boy_map_2.png') }}"
                    });

                    google.maps.event.addListener(dmmarker, 'click', (function(dmmarker) {
                        return function() {
                            infowindow.setContent(
                                `<div class='float--left'><img class='js--design-1 mt-2' onerror="this.src='{{ asset('assets/admin/img/160x160/img1.jpg') }}'"  src='{{ asset('storage/app/delivery-man/' . $order->delivery_man->image) }}'></div><div class='float--right p--10px'><b>{{ $order->delivery_man->f_name }}  {{ $order->delivery_man->l_name }}</b><br/>  <div> {{ translate('messages.Active_Orders') }} : {{ $order->delivery_man->current_orders }} </div> </b> {{ $order->dm_last_location['location'] }}</div>`
                            );
                ...
                @if ($order->restaurant)
                    ...
                    google.maps.event.addListener(Retaurantmarker, 'click', (function(Retaurantmarker) {
                        return function() {
                            infowindow.setContent(
                                `<div class='float--left'><img class='js--design-1' onerror="this.src='{{ asset('assets/admin/img/160x160/img1.jpg') }}'"  src='{{ asset('storage/app/restaurant/' . $order->restaurant->logo) }}'></div><div class='float--right p--10px'><b>{{ Str::limit($order->restaurant->name, 15, '...') }}</b><br/> {{ $order->restaurant->address }}</div>`
                            );

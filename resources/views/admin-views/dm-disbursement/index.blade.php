...
<div class="content container-fluid">
    <div class="page-header">
        ...
        <ul class="nav nav-tabs mb-4 border-0 pt-2">
            <li class="nav-item">
                <a class="nav-link {{ $status == 'all'?'active':'' }}" href="{{ route('admin.dm-disbursement.list', ['status' => 'all']) }}" >{{ translate('all') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ $status == 'pending'?'active':'' }}" href="{{ route('admin.dm-disbursement.list', ['status' => 'pending']) }}">{{ translate('pending') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ $status == 'processing'?'active':'' }}" href="{{ route('admin.dm-disbursement.list', ['status' => 'processing']) }}">{{ translate('processing') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ $status == 'completed'?'active':'' }}" href="{{ route('admin.dm-disbursement.list', ['status' => 'completed']) }}">{{ translate('completed') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ $status == 'partially_completed'?'active':'' }}" href="{{ route('admin.dm-disbursement.list', ['status' => 'partially_completed']) }}">{{ translate('partially_completed') }}</a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ $status == 'canceled'?'active':'' }}" href="{{ route('admin.dm-disbursement.list', ['status' => 'canceled']) }}">{{ translate('canceled') }}</a>
            </li>
        </ul>
    </div>
    <!-- Reports -->
            ...
            <div class="card">
                <div class="card-header border-0 flex-wrap justify-content-between gap-4">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-2">
                        ...
                        <div>
                            <a href="{{ route('admin.dm-disbursement.view', ['id' => $disbursement->id]) }}" class="btn btn--primary">{{ translate('view_details') }}</a>
                        </div>
                    </div>
                </div>
            </div>

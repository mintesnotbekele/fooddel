    ...
    <table class="table table-align-middle">
        ...
        @if(session()->has('cart') && count( session()->get('cart')) > 0)
            ...
            @if(is_array($cartItem))
                ...
                ?>
            <tr>
                <td class="media cart--media align-items-center cursor-pointer" onclick="quickViewCartItem({{$cartItem['id']}}, {{$key}})">
                    <img class="avatar avatar-sm mr-2" src="{{asset('storage/app/product')}}/{{$cartItem['image']}}"

...
//        $vendor['withdraw_able_balance'] =(float) ($vendor?->wallet?->balance  > 0 ? abs($vendor?->wallet?->balance): 0) ;
        ...
        elseif($vendor?->wallet?->balance ==  $wallet_earning  ){
            ...
            $vendor['Balance_type'] = 'Adjusted';

        }
        else{
            $vendor['adjust_able'] = false;
            $vendor['Balance_type'] = 'Unadjusted';
        }

        $vendor['show_pay_now_button'] = false;

        if ($min_amount_to_pay_restaurant <= $vendor?->wallet?->collected_cash){
        ...
        if($Payable_Balance == 1 &&  $cash_in_hand_overflow &&  $vendor?->wallet?->balance < 0 &&  $val <=  abs($vendor?->wallet?->balance)  &&  $cash_in_hand_overflow_restaurant_amount >= abs($vendor?->wallet?->balance)){

            $vendor['over_flow_warning'] = true;
        }

        $vendor['over_flow_block_warning'] = false;
        if ($Payable_Balance == 1 &&  $cash_in_hand_overflow &&  $vendor?->wallet?->balance < 0 &&  $cash_in_hand_overflow_restaurant_amount < abs($vendor?->wallet?->balance)){
